...
is.factor(data1)
sapply(data1,class)
data1$PoolQC=as.numeric(data1$PoolQC)
class(PoolQC)
data1$PoolQC=as.numeric(data1$PoolQC)
is.numeric(data1$PoolQC)
class(PoolQC)
data1$GarageType=as.numeric(data1$GarageType)
is.numeric(Garage.type)
is.numeric(Garage.type)
is.numeric(data1$GarageType)
as.factor(data1$GarageType)
data1$GarageType <- as.factor(data1$GarageType)
View(data1)
View(data1)
data1$GarageType <- as.numeric(data1$GarageType)
View(data1)
summary(data1)
data1$GarageType <- as.factor(data1$GarageType)
is.factor(data1$GarageType)
data1$GarageFinish <- as.factor(data1$GarageFinish)
is.factor(data1$GarageFinish)
View(data1)
data1$GarageFinish <- as.numeric(data1$GarageFinish)
is.numeric(data1$GarageFinish)
View(data1)
data1$GarageFinish <- as.factor(data1$GarageFinish)
is.factor(data1$GarageFinish)
attach(data1)
cor(FullBath,SalePrice)
cor(HalfBath,SalePrice)
plot(FullBath,SalePrice)
plot(HalfBath,SalePrice)
plot(FullBath,SalePrice)
barplot(KitchenQual,SalePrice)
data1$KitchenQual <- as.factor(data1$KitchenQual)
barplot(KitchenQual,SalePrice)
is.factor(KitchenQual)
View(data1)
data1$KitchenQual <- as.numeric(data1$KitchenQual)
barplot(KitchenQual,SalePrice)
cor(KitchenQual,SalePrice)
is.numeric(KitchenQual)
data1$KitchenQual <- as.numeric(data1$KitchenQual)
is.numeric(KitchenQual)
data1$KitchenQual <- as.numeric(data1$KitchenQual)
is.numeric(data1$KitchenQual)
cor(KitchenQual,SalePrice)
attach(data1)
detach(data1)
detach(data1)
attach(data1)
is.numeric(KitchenQual)
cor(SalePrice,KitchenQual)
cor(SalePrice,Fireplaces)
cor(SalePrice,X1stFlrSF)
plot(SalePrice,X1stFlrSF)
cor(SalePrice,X2ndFlrSF)
cor(SalePrice,GrLivArea)
plot(SalePrice,GrLivArea)
?cor
cor(SalePrice,BsmtUnfSF)
cor(SalePrice,TotalBsmtSF)
plot(SalePrice,TotalBsmtSF)
plot(SalePrice,YearBuilt)
View(data1)
data1$YearBuilt <- as.numeric(data1$YearBuilt)
cor(SalePrice,YearBuilt)
plot(SalePrice,YearRemodAdd)
data1$YearBuilt <- as.numeric(data1$YearRemodAdd)
cor(SalePrice,YearBuilt)
data1$YearRemodAdd <- as.numeric(data1$YearRemodAdd)
cor(SalePrice,YearRemodAdd)
plot(SalePrice,HouseStyle)
data1$HouseStyle <- as.numeric(data1$HouseStyle)
cor(SalePrice,HouseStyle)
data1$HouseStyle <- as.numeric(data1$HouseStyle)
cor(SalePrice,HouseStyle)
is.numeric(HouseStyle)
data1$HouseStyle <- as.numeric(data1$HouseStyle)
is.numeric(HouseStyle)
View(data1)
is.numeric(HouseStyle)
...
is.numeric()
is.numeric(HouseStyle)
is.factor(HouseStyle)
data1$HouseStyle <- as.numeric(data1$HouseStyle)
is.numeric(HouseStyle)
class(data1$OverallQual)
cor(SalePrice, OverallQual)
plot(SalePrice,OverallQual)
cor(SalePrice, OverallCond)
plot(SalePrice,OverallCond)
plot(SalePrice,OverallQual)
plot(SalePrice,OverallCond)
plot(SalePrice,Neighborhood)
data1$HouseStyle <- as.numeric(data1$HouseStyle)
abline(lm(SalePrice~HouseStyle))
abline(lm(HouseStyle~SalePrice))
plot(SalePrice,LotConfig)
abline(lm(LotConfig~SalePrice))
data1$LotConfig <- as.numeric(data1$LotConfig)
as.integer(LotConfig)
cor(SalePrice, LotConfig)
cor(LotConfig,SalePrice)
class(LotConfig)
barplot(SalePrice,LotConfig)
?barplot
piechart(LotConfig)
pie(LotConfig)
pie
...
data1[,c(1,2)]
data1[c(1,2),]
pairs(data1[,c("garagecars","GarageArea","KitchenAbvGr",)])
pairs(data1[,c("garagecars","GarageArea","KitchenAbvGr")])
pairs(data1[c("garagecars","GarageArea","KitchenAbvGr")])
pairs(data1[,c("garagecars","GarageArea","KitchenAbvGr")])
pairs(data1[,c("garagecars","GarageArea")])
plot(GarageArea,GarageCars)
mod<-lm(data1~MSSubClass,	ScreenPorch,	PoolArea,	PoolQC,	Fence,	MiscFeature,GarageType,	GarageYrBlt,	GarageFinish	,ExterQual	,ExterCond	,Foundation	,YearBuilt	,YearRemodAdd,	RoofStyle,	RoofMatl,	Exterior1st	,BldgType,	HouseStyle,	OverallQual,	OverallCond,	Neighborhood,	Condition1,	Condition2	,LandContour	,Utilities	,LotConfig	,LandSlope	,LotArea,	Street,	Alley	,LotShape,	MSZoning	,LotFrontage,	MoSold	,YrSold,	SaleType,	SaleCondition,	SalePrice
)
summary(mod)
?lm
mod<-lm(data1~MSSubClass+ScreenPorch+PoolArea+PoolQC+Fence+MiscFeature+GarageType+GarageYrBlt+GarageFinish	+ExterQual	+ExterCond	+Foundation	
...
list(data1)
mod1<-lm(data1~ExterQual,OverallQual)
mod1<-lm(data1~ExterQual+OverallQual)
mod1<-lm(data~ExterQual+OverallQual)
mod1<-lm(data1~ExterQual+OverallQual)
data1<-read.csv(file.choose(),header = T)
data<-read.table(file.choose(),header = T,sep = "\t")
mod1<-lm(data1~ExterQual+OverallQual)
mod1<-lm(SalePrice ~ MSSubClass + ScreenPorch + PoolArea, data = data1)
mod2<-lm(SalePrice~....,data=data1)
mod2<-lm(SalePrice~...elt(80),data=data1)
mod2<-lm(SalePrice~...elt(81),data=data1)
mod2<-lm(SalePrice~...length(),data=data1)
mod2<-lm(SalePrice~...length(80),data=data1)
mod2<-lm(SalePrice~...length(81),data=data1)
mod2<-lm(SalePrice~... = 80,data=data1)
mod2<-lm(SalePrice~...80,data=data1)
mod2<-lm(SalePrice~...salecondition,data=data1)
mod2<-lm(SalePrice~...SaleCondition,data=data1)
mod2<-lm(SalePrice~...elt(,),data=data1)
mod2<-lm(SalePrice~...elt(1,),data=data1)
mod2<-lm(SalePrice~...elt(1,80),data=data1)
mod2<-lm(SalePrice~...,data=data1)
mod2<-lm(SalePrice~..getNamespace(SaleCondition,80),data=data1)
mod2<-lm(SalePrice~MSSubClass...,data=data1)
mod2<-lm(SalePrice~MSSubClass+...,data=data1)
mod2<-lm(SalePrice~.,data=data1)
mod2<-lm(SalePrice ~.,data1)
mod2<-lm(SalePrice ~ .,data1)
mod2 <- lm(SalePrice ~ . , data1)
mod2 <- lm(SalePrice ~ . , data = data1)
any(is.na(data1))
...
colSums(is.na(data7))
              ...
              view(dataA)
                ...
                sapply(data_n,class)
                data_n[is.na(data_n)]<-mean(data_f[!is.na(data_n)])
                data_n[is.na(data_n)]<-mean(data_n[!is.na(data_n)])
                data_n
                data_f[is.na(data_f)]<-median(data_f[!is.na(data_f)])
                data_f[is.na(data_f)]<-mode(data_f[!is.na(data_f)])
                data_f
                ...
                is.factor(data_f)
                ?as.factor
                as.factor(data_f$GarageQual)
                View(data_f)
                is.factor(GarageQual)
...
# scaling a variable:
max_<-apply(data1,2,max)  # here 2 represent column (and  represent row):
min_<-apply(data1,2,min)
scaled_<-as.data.frame(scale(data1,center=min_,scale=max_-min_))
